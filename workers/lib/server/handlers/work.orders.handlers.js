...
const {
  renderWorkOrderCsv,
  renderWorkOrdersBulkCsv,
  renderRmaCsv,
} = require("../lib/work.order.export");
const {
...
async function _resolvePartByIdentifier(ctx, identifier) {
  ...
  return (
    flattenRpcResults(results).find((t) => t?.type !== WORK_ORDER_THING_TYPE) ||
    null
  );
}
...
async function _resolveReplacement(
  ctx,
  { type, deviceType, part, toLocation, identifier, movingIds, usedIds },
) {
  if (type !== WORK_ORDER_TYPES.MOVE)
    throw _badRequest("ERR_WO_REPLACEMENT_NOT_ALLOWED");
  if (deviceType !== "miner")
    throw _badRequest("ERR_WO_REPLACEMENT_DEVICE_TYPE_INVALID");
  if (!toLocation || toLocation === MINER_ROOM_LOCATION)
  ...
  if (!_occupiesSocket(part))
    throw _badRequest("ERR_WO_REPLACEMENT_POSITION_UNKNOWN");
  ...
  if (!_isMiner(replacement))
    throw _badRequest("ERR_WO_REPLACEMENT_DEVICE_TYPE_INVALID");
  if (movingIds.has(replacement.id))
    throw _badRequest("ERR_WO_REPLACEMENT_IS_MOVING");
  if (usedIds.has(replacement.id))
    throw _badRequest("ERR_WO_REPLACEMENT_DUPLICATE");
  if (
    replacement.info?.location === MINER_ROOM_LOCATION ||
    _occupiesSocket(replacement)
  ) {
  ...
  return {
    thing: replacement,
    identifier,
    vacated: { container, pos, subnet: subnet ?? null },
  };
}
...
async function _moveAttachedParts(
  ctx,
  req,
  { miner, deviceType, toLocation, woId, voter, ts },
) {
  if (deviceType !== "miner" || toLocation == null) return [];
  ...
  const parts = flattenRpcResults(results).filter(
    (t) => t?.type !== WORK_ORDER_THING_TYPE,
  );
  ...
  for (const part of parts) {
    ...
    assertActionApplied(
      partResults,
      `ERR_ATTACHED_PART_MOVE_PUSH_FAILED:${part.id}`,
    );
    moves.push({
      partId: part.id,
      partCode: part.code,
...
async function createWorkOrder(ctx, req) {
  ...
  if (
    type === WORK_ORDER_TYPES.MICROBT_MINER ||
    type === WORK_ORDER_TYPES.MICROBT_NON_MINER
  ) {
...
<<<<<<< HEAD
      const partResults = await submitWorkOrderAction(
        ...
        req,
        "updateThing",
        { id: part.id, info: { status: info.deviceStatus, workOrderId: woId } },
        part.rack,
      );
...
>>>>>>> e1196a492e348780ddc4fdd97bb092bea6c537a2
    ...
    const placement =
      info.location != null
        ? _minerPlacementInfo(deviceType, info.location, info)
        : {};
    ...
    if (info.location != null) {
      ...
      assertActionApplied(partResults, "ERR_PART_MOVE_PUSH_FAILED");
      info.partsMoves.push(
        ...(await _moveAttachedParts(ctx, req, {
          miner: part,
          deviceType,
          toLocation: info.location,
          woId,
    ...
    if (replacement) {
      ...
      const replacementResults = await submitWorkOrderAction(
        ctx,
        req,
        "updateThing",
        {
...
async function createWorkOrdersBatch(ctx, req) {
  ...
  const hasRepairMinerIdentifier =
    isMinerRepair && Boolean(extraInfo?.minerIdentifier);
  const minerToRepair = hasRepairMinerIdentifier
    ? await _resolvePartByIdentifier(ctx, extraInfo.minerIdentifier)
    : null;
  if (hasRepairMinerIdentifier && !minerToRepair)
    throw _badRequest("ERR_PART_NOT_FOUND");
  ...
  const summary = minerToRepair
    ? {
        ...
        deviceIdentifier:
          minerToRepair.info?.serialNum ??
          minerToRepair.code ??
          extraInfo.minerIdentifier,
      }
    : devices[0];
  ...
  for (const { device, part, replacement } of resolved) {
    const placement =
      type === WORK_ORDER_TYPES.MOVE && info.location != null
        ? _minerPlacementInfo(device.deviceType, info.location, device)
        : {};
    const move = _buildPartsMove(
      type,
      part,
      device,
      info,
...
<<<<<<< HEAD
    ...
    const minerResults = await submitWorkOrderAction(
      ...
      {
        id: minerToRepair.id,
        info: { status: info.deviceStatus, workOrderId: woId },
      },
      minerToRepair.rack,
    );
...
async function listWorkOrders(ctx, req) {
  ...
  return listThingsWithCount(ctx, query, {
    ...
    sort:
      req.query.sort &&
      parseJsonQueryParam(req.query.sort, "ERR_SORT_INVALID_JSON"),
    fields:
      req.query.fields &&
...
async function getWorkOrder(ctx, req) {
  ...
  if (!wo.info?.deviceCode && identifier) {
    const part = await _resolvePartByIdentifier(ctx, identifier).catch(
      () => null,
    );
...
async function appendWorkLogEntry(ctx, req) {
  ...
  return ctx.dataProxy.requestData(
    "saveThingComment",
    {
      rackId,
...
async function exportWorkOrdersRma(ctx, req, rep) {
  const wos = (await _loadWorkOrdersByIdsOrCodes(ctx, req.query.ids)).filter(
    (wo) => wo?.info?.type === WORK_ORDER_TYPES.MICROBT_MINER,
  );
