...
const mockAuthLib = {
  getTokenPerms: async () => ({
    permissions: ["inventory:rw", "work_order:rw", "actions:rw"],
  }),
};
    ...
    parts: [
      {
        ...
        code: "PSU-1",
        type: "inventory-miner_part-psu",
        info: { serialNum: "SN-1", location: "site.lab" },
      },
    ],
  });
  await handlers.createWorkOrder(flow.ctx, {
...
test("handlers: createWorkOrder Type 2 (move) relocates the part on its own rack", async (t) => {
  ...
  t.is(
    partPush.params[0].rackId,
    "psu-rack-1",
    "relocation targets the part rack",
  );
  t.is(
    partPush.params[0].info.location,
    "site.warehouse",
    "part moved to the destination",
  );
  t.ok(
    partPush.params[0].info.workOrderId,
    "relocation carries a workOrderId (part-move gate)",
  );
  t.is(
    partPush.params[0].info.workOrderId,
    regPush.params[0].id,
    "relocation references the created WO id",
  );
});
...
test("handlers: createWorkOrder Type 2 (move) surfaces a failed relocation push instead of swallowing it", async (t) => {
  const ctx = createMockCtxWithOrks(
    ...
    async (_k, method, params) => {
      if (method === "pushAction") {
        if (params.action === "updateThing")
          return { id: null, errors: ["ERR_ORK_ACTION_CALLS_EMPTY"] };
        return { id: "a", errors: [] };
      }
      if (method === "listThings")
        return [
        ...
        return [
          {
            ...
            action: {
              targets: {
                "miner-rack-1": {
                  calls: [
                    { id: "miner-1", error: "[HRPC_ERR]=ERR_SUBNET_NOT_FOUND" },
                  ],
                },
              },
            },
    ...
    () =>
      handlers.createWorkOrder(ctx, {
        ...
        body: {
          ...
          info: { location: "miner.room", pos: "1-4_1", container: "group-1" },
        },
      }),
    /ERR_WO_DEVICE_UPDATE_FAILED.*ERR_SUBNET_NOT_FOUND/,
  );
  t.absent(
    pushed.find((p) => p.action === "registerThing"),
    "no work order is registered when the device move failed",
  );
});
...
test("handlers: createWorkOrder Type 2 (move) registers the work order when the executed action carries no error", async (t) => {
        ...
        return [
          {
            ...
            action: {
              targets: {
                "miner-rack-1": { calls: [{ id: "miner-1", result: 1 }] },
              },
            },
          },
        ];
  ...
  t.ok(
    pushed.find((p) => p.action === "registerThing"),
    "work order is registered once the move applied",
  );
});
...
test("handlers: createWorkOrdersBatch Type 2 (move) relocates every part", async (t) => {
  ...
  const ctx = createMockCtxWithOrks(
    ...
    async (_k, method, params) => {
      ...
      if (method === "listThings") {
        const sn = (params.query?.$or || [])
          .map((c) => c["info.serialNum"])
          .find(Boolean);
        return [
          {
            id: sn,
  ...
  t.ok(
    partPushes.every(
      (p) => p.params[0].info.workOrderId === regPush.params[0].id,
    ),
    "every relocation references the created WO id",
  );
});
...
function buildMinerMoveCtx(pushed, minerInfo) {
  const ctx = createMockCtxWithOrks(
    ...
    async (_k, method, params) => {
      ...
      if (method === "listThings" && params.query?.["info.parentDeviceId"])
        return [];
      if (method === "listThings")
        return [
          {
...
const ATTACHED = [
  {
    ...
    rack: "cb-rack-1",
    info: { location: "miner.room", parentDeviceId: "miner-1" },
  },
  {
    id: "part-2",
    code: "PS-1",
...
test("handlers: createWorkOrder Type 2 (move) relocates every attached part with the miner", async (t) => {
  ...
  const regPush = pushed.find((p) => p.action === "registerThing");
  const partPushes = pushed.filter(
    (p) => p.action === "updateThing" && p.params[0].rackId !== "miner-rack-1",
  );
  t.is(partPushes.length, 2, "one relocation per attached part");
  t.alike(
    partPushes.map((p) => p.params[0].rackId),
    ["cb-rack-1", "psu-rack-1"],
    "each part moves on its own rack",
  );
  ...
  t.ok(
    ...
    "relocations carry the WO id",
  );
  const attached = regPush.params[0].info.partsMoves.filter(
    (m) => m.role === "attached",
  );
  t.is(attached.length, 2, "one movement record per part, written once");
  t.alike(
    attached.map((m) => m.partCode),
    ["CB-1", "PS-1"],
  );
...
test("handlers: createWorkOrder Type 2 (move) leaves unattached parts alone and moves a miner with none", async (t) => {
  ...
  const regPush = pushed.find((p) => p.action === "registerThing");
  t.is(
    pushed.filter((p) => p.action === "updateThing").length,
    1,
    "only the miner moves",
  );
...
test("handlers: createWorkOrder Type 2 (move) miner leaving miner.room clears pos and parks it in maintenance", async (t) => {
  const pushed = [];
  const ctx = buildMinerMoveCtx(pushed, {
    location: "miner.room",
    container: "group-3",
    pos: "1_2",
  });
  ...
  t.is(
    minerPush.params[0].info.container,
    "maintenance",
    "container parked at maintenance",
  );
...
test("handlers: createWorkOrder Type 2 (move) applies deviceStatus to the device and records it in partsMoves", async (t) => {
  ...
  t.is(
    regPush.params[0].info.status,
    undefined,
    "deviceStatus does not leak into the WO workflow status",
  );
  const move = regPush.params[0].info.partsMoves[0];
...
test("handlers: createWorkOrder Type 2 (move) omits device status when no deviceStatus is sent", async (t) => {
  ...
  const minerPush = pushed.find((p) => p.action === "updateThing");
  t.is(
    minerPush.params[0].info.status,
    undefined,
    "no status pushed without deviceStatus",
  );
});
...
test("handlers: createWorkOrdersBatch Type 2 (move) applies the shared deviceStatus to every device", async (t) => {
        ...
        return [
          {
            ...
            type: "miner-whatsminer",
            rack: "miner-rack-1",
            info: { location: "miner.room", status: "in_operation" },
          },
        ];
      }
      return null;
  ...
  t.ok(
    regPush.params[0].info.partsMoves.every(
      (m) => m.fromStatus === "in_operation" && m.toStatus === "faulty",
    ),
  );
});
...
test("handlers: createWorkOrder Type 2 (move) leaves parts untouched by miner placement rules", async (t) => {
  ...
  t.is(
    partPush.params[0].info.container,
    undefined,
    "no container on part relocation",
  );
});
  ...
  const ctx = createMockCtxWithOrks(
    ...
    async (_k, method, params) => {
      ...
      if (method === "listThings" && params.query?.["info.parentDeviceId"])
        return [];
      if (method === "listThings") {
        const wanted = (params.query?.$or || [])
          .map(
            (c) =>
              c.id || c.code || c["info.serialNum"] || c["info.macAddress"],
          )
          .find(Boolean);
        return things.filter((t) =>
          [t.id, t.code, t.info?.serialNum, t.info?.macAddress].includes(
            wanted,
          ),
        );
      }
  ...
  return t
    ...
    .then(() =>
      t.is(
        pushed.length,
        0,
        "nothing is moved when the replacement is rejected",
      ),
    );
};
...
test("handlers: createWorkOrdersBatch Type 2 (move) installs the replacement miner into the vacated socket", async (t) => {
  ...
  t.is(
    outPush.params[0].id,
    "miner-1",
    "outgoing miner is pushed first, freeing the socket",
  );
  t.is(outPush.params[0].info.pos, "", "outgoing pos cleared");
  t.is(inPush.params[0].id, "miner-2");
  t.is(
    inPush.params[0].rackId,
    "miner-rack-2",
    "replacement is updated on its own rack",
  );
  t.is(inPush.params[0].info.location, "miner.room");
  t.is(
    inPush.params[0].info.container,
    "group-3",
    "replacement takes the vacated group",
  );
  t.is(
    inPush.params[0].info.pos,
    "3_1",
    "replacement takes the vacated socket",
  );
  t.is(
    inPush.params[0].info.subnet,
    "10.0.3.0",
    "replacement inherits the group subnet",
  );
  t.is(
    inPush.params[0].info.workOrderId,
    regPush.params[0].id,
    "replacement references the created WO id",
  );
  t.is(
    inPush.params[0].info.status,
    "in_operation",
    "replacement goes in operation with the same update",
  );
...
test("handlers: createWorkOrder Type 2 (move) accepts a replacement via info.replacementIdentifier", async (t) => {
  ...
  const moves = pushed.find((p) => p.action === "registerThing").params[0].info
    .partsMoves;
      ...
      devices: [
        {
          ...
          deviceModel: "M56",
          deviceIdentifier: "SN-OUT",
          replacementIdentifier: "SN-SPARE",
        },
      ],
      issue: "fan stopped",
    },
      ...
      devices: [
        {
          deviceType: "psu",
          deviceModel: "P",
          deviceIdentifier: "SN-PSU",
          replacementIdentifier: "SN-SPARE",
        },
      ],
      info: { location: "site.lab" },
    },
    /ERR_WO_REPLACEMENT_DEVICE_TYPE_INVALID/,
    [psu, SPARE],
  );
});
...
test("handlers: replacement 400s when the move does not empty a socket", async (t) => {
  await rejects(
    t,
    moveOutBody(
      { location: "miner.room", container: "group-4", pos: "4_1" },
      { replacementIdentifier: "SN-SPARE", container: "group-4", pos: "4_1" },
    ),
    /ERR_WO_REPLACEMENT_NOT_LEAVING_MINER_ROOM/,
  );
  const parked = {
    ...OUTGOING,
    info: { ...OUTGOING.info, location: "site.warehouse" },
  };
  await rejects(
    t,
    moveOutBody({}, { replacementIdentifier: "SN-SPARE" }),
    /ERR_WO_REPLACEMENT_NOT_LEAVING_MINER_ROOM/,
    [parked, SPARE],
  );
  ...
  const noSocket = {
    ...OUTGOING,
    info: { ...OUTGOING.info, container: "maintenance", pos: "" },
  };
  await rejects(
    t,
    moveOutBody({}, { replacementIdentifier: "SN-SPARE" }),
    /ERR_WO_REPLACEMENT_POSITION_UNKNOWN/,
    [noSocket, SPARE],
  );
});
...
test("handlers: replacement 400s when the replacement miner cannot be used", async (t) => {
  await rejects(
    t,
    moveOutBody({}, { replacementIdentifier: "SN-NOPE" }),
    /ERR_WO_REPLACEMENT_NOT_FOUND/,
    [OUTGOING],
  );
  ...
  const psu = {
    ...
    type: "inventory-miner_part-psu",
    rack: "psu-rack-1",
    info: { serialNum: "SN-PSU", location: "site.warehouse" },
  };
  await rejects(
    t,
    moveOutBody({}, { replacementIdentifier: "SN-PSU" }),
    /ERR_WO_REPLACEMENT_DEVICE_TYPE_INVALID/,
    [OUTGOING, psu],
  );
  ...
  await rejects(
    t,
    moveOutBody({}, { replacementIdentifier: "SN-SPARE" }),
    /ERR_WO_REPLACEMENT_NOT_AVAILABLE/,
    [OUTGOING, inRoom],
  );

  const socketed = {
    ...SPARE,
    info: { serialNum: "SN-SPARE", container: "group-9", pos: "9_1" },
  };
  await rejects(
    t,
    moveOutBody({}, { replacementIdentifier: "SN-SPARE" }),
...
test("handlers: replacement 400s when it is itself moving out or reused twice", async (t) => {
      ...
      devices: [
        {
          ...
          deviceModel: "M56",
          deviceIdentifier: "SN-OUT",
          replacementIdentifier: "SN-OUT-2",
        },
        {
          deviceType: "miner",
          deviceModel: "M56",
          deviceIdentifier: "SN-OUT-2",
        },
      ],
      info: { location: "site.lab" },
    },
    /ERR_WO_REPLACEMENT_IS_MOVING/,
    [OUTGOING, second],
  );
  ...
  await rejects(
    ...
    /ERR_WO_REPLACEMENT_DUPLICATE/,
    [OUTGOING, second, SPARE],
  );
});
...
test("handlers: replacement surfaces a failed push instead of swallowing it", async (t) => {
  const ctx = createMockCtxWithOrks(
    ...
    async (_k, method, params) => {
      if (method === "pushAction") {
        if (params.params[0].id === "miner-2")
          return { id: null, errors: ["ERR_ORK_ACTION_CALLS_EMPTY"] };
        return { id: "a", errors: [] };
      }
      if (method === "listThings") {
        const wanted = (params.query?.$or || [])
          .map((c) => c.id || c.code || c["info.serialNum"])
          .find(Boolean);
        return [OUTGOING, SPARE].filter((t) =>
          [t.id, t.code, t.info?.serialNum].includes(wanted),
        );
      }
      return null;
    },
        ...
        return [
          {
            ...
            type: "miner-whatsminer",
            rack: "miner-rack-1",
            info: { serialNum: "AM-1", status: "faulty" },
          },
        ];
      return null;
    },
...
test("handlers: createWorkOrdersBatch Type 3 keeps the miner as the root subject, not the first spare part", async (t) => {
      ...
      devices: [
        {
          ...
          deviceIdentifier: "QAHB01-WM63SPP00750",
        },
        {
          deviceType: "hashboard",
          deviceModel: "miner-wm-m63spp",
          deviceIdentifier: "QAHB02-WM63SPP00750",
        },
      ],
      info: {
        notes: "Miner SN: WM63SPP00750",
        minerIdentifier: "pirxAnVkFzZLTEZ",
      },
    },
  });
  ...
  t.is(
    info.deviceType,
    "miner",
    "root subject is the miner, not the hashboard",
  );
        ...
        return [
          {
            ...
            type: "miner-whatsminer",
            rack: "miner-rack-1",
            info: { location: "miner.room", container: "group-2", pos: "3_4" },
          },
        ];
      }
      return null;
...
test("handlers: createWorkOrdersBatch builds one WO with a parts-move per device, first device as summary", async (t) => {
  const parts = [
    {
      ...
      code: "WMM-1",
      type: "inventory-miner_part-controller",
      info: { serialNum: "WMM63S-2024-04829", location: "site.warehouse" },
    },
    {
      id: "part-2",
      code: "WMM-2",
      type: "inventory-miner_part-controller",
      info: { serialNum: "WMM63S-2024-04830", location: "site.warehouse" },
    },
    {
      id: "part-3",
      code: "WMM-3",
      type: "inventory-miner_part-controller",
      info: { serialNum: "WMM63S-2024-04831", location: "site.warehouse" },
    },
  ];
  let lastPush;
  const handler = async (_key, method, params) => {
    ...
    if (method === "listThings") {
      ...
      const sn = or
        .map(
          (c) => c.id || c.code || c["info.serialNum"] || c["info.macAddress"],
        )
        .find(Boolean);
  ...
  await handlers.createWorkOrdersBatch(ctx, {
    ...
    body: {
      ...
      devices: [
        {
          deviceType: "miner",
          deviceModel: "whatsminer-m63s",
          deviceIdentifier: "WMM63S-2024-04829",
        },
        {
          deviceType: "miner",
          deviceModel: "whatsminer-m63s",
          deviceIdentifier: "WMM63S-2024-04830",
        },
        {
          deviceType: "miner",
          deviceModel: "whatsminer-m63s",
          deviceIdentifier: "WMM63S-2024-04831",
        },
      ],
      info: { location: "site.miner-room" },
    },
  ...
  t.is(
    info.deviceIdentifier,
    "WMM63S-2024-04829",
    "first device is the summary identifier",
  );
  t.is(info.partsMoves.length, 3, "one parts-move per device");
  t.alike(
    info.partsMoves.map((m) => m.deviceIdentifier),
    ["WMM63S-2024-04829", "WMM63S-2024-04830", "WMM63S-2024-04831"],
  );
  t.alike(
    info.partsMoves.map((m) => m.partId),
    ["part-1", "part-2", "part-3"],
    "each move resolves its own part",
  );
  t.is(info.partsMoves[0].role, "move");
  t.is(info.partsMoves[0].fromLocation, "site.warehouse");
  t.is(
    info.partsMoves[0].toLocation,
    "site.miner-room",
    "all moved to the WO target location",
  );
});
...
test("handlers: updateWorkOrder forwards warranty payload to updateThing", async (t) => {
  ...
  await handlers.updateWorkOrder(flow.ctx, {
    ...
    body: {
      warranty: {
        vendor: "microbt",
        fields: { rmaNumber: "RMA-1", faultCode: "E03" },
      },
    },
  });
...
test("handlers: listWorkOrders returns paginated envelope with type pinned", async (t) => {
  ...
  const out = await handlers.listWorkOrders(flow.ctx, {
    query: { offset: 0, limit: 2 },
  });
  t.is(
    flow.lastList.query.type,
    "inventory-work_order",
    "list pinned to WO type",
  );
  t.is(
    flow.lastCount.query.type,
    "inventory-work_order",
    "count pinned to WO type",
  );
  t.alike(
    out.data.map((o) => o.id),
    ["a", "b"],
...
test("handlers: listWorkOrders ?q builds a regex $or against code, info.issue, and operator emails", async (t) => {
  ...
  t.alike(or[3], {
    "info.assignedTo": { $regex: "IVI-2-0001", $options: "i" },
  });
});
...
test("handlers: listWorkOrders shortcuts map to mingo paths", async (t) => {
  ...
  t.alike(flow.lastList.query["info.createdAt"], {
    $gte: 1700000000000,
    $lte: 1700864000000,
  });
});
...
test("handlers: getWorkOrder filters by id+type and 404s when nothing found", async (t) => {
  const ctx = createMockCtxWithOrks(
    [{ rpcPublicKey: "k" }],
    async (_k, _m, params) =>
      params.query?.id === "found" ? [{ id: "found", code: "IVI-2-0001" }] : [],
  );
...
test("handlers: appendWorkLogEntry calls saveThingComment with the right rack/thingId/user", async (t) => {
  ...
  const ctx = createMockCtxWithOrks(
    ...
    async (_k, method, params) => {
      if (method === "listThings")
        return [{ id: "wo-1", info: { status: "open" } }];
...
test("handlers: exportWorkOrder pdf returns 501 (deferred to phase 2)", async (t) => {
  ...
  const rep = mkRep();
  await handlers.exportWorkOrder(
    ctx,
    { params: { id: "IVI-2-0001" }, query: { format: "pdf" } },
    rep,
  );
  t.is(rep._status, 501);
  t.ok(
    /^ERR_EXPORT_FORMAT_NOT_IMPLEMENTED:(pdf|docx)$/.test(rep._body.message),
  );
});
...
test("handlers: exportWorkOrder docx returns 501 (deferred to phase 2)", async (t) => {
  ...
  await handlers.exportWorkOrder(
    ctx,
    { params: { id: "IVI-2-0001" }, query: { format: "docx" } },
    rep,
  );
  t.is(rep._status, 501);
  t.ok(
...
test("handlers: exportWorkOrder 404s when WO not found by id or code", async (t) => {
  ...
  await t.exception(
    () =>
      handlers.exportWorkOrder(
        ctx,
        { params: { id: "nope" }, query: { format: "csv" } },
        mkRep(),
      ),
    /ERR_WORK_ORDER_NOT_FOUND/,
  );
...
test("handlers: exportWorkOrdersRma returns CSV of only the MicroBT Miner WOs selected", async (t) => {
  ...
  const rep = mkRep();
  await handlers.exportWorkOrdersRma(
    ctx,
    { query: { ids: "IVI-3-0001,IVI-2-0002" } },
    rep,
  );
  t.is(rep._headers["content-type"], "text/csv; charset=utf-8");
...
test("handlers: exportWorkOrdersBulk returns one combined CSV for every selected WO regardless of type", async (t) => {
  const register = {
    id: "wo-1",
    code: "IVI-1-0001",
    info: { type: 1, deviceType: "psu", partsMoves: [] },
  };
  const repair = {
    id: "wo-3",
    code: "IVI-3-0001",
    info: {
      ...
      deviceModel: "M63S++_VL28",
      issue: "low hashrate",
      partsMoves: [{ role: "diagnosis", partCode: "HB-OLD" }],
    },
  };
  const ctx = createMockCtxWithOrks([{ rpcPublicKey: "k" }], async () => [
    register,
  ...
  const rep = mkRep();
  await handlers.exportWorkOrdersBulk(
    ctx,
    { query: { ids: "IVI-1-0001,IVI-3-0001" } },
    rep,
  );
  t.is(rep._headers["content-type"], "text/csv; charset=utf-8");
...
test("handlers: exportWorkOrdersBulk unions headers across differently-shaped work order types", async (t) => {
  ...
  const repair = {
    id: "wo-3",
    code: "IVI-3-0001",
    info: { type: 3, issue: "low hashrate", partsMoves: [] },
  };
  const ctx = createMockCtxWithOrks([{ rpcPublicKey: "k" }], async () => [
    register,
    repair,
  ...
  t.ok(
    header.includes("deviceType"),
    "register-only field present in union header",
  );
...
test("handlers: exportWorkOrdersBulk emits one row per partsMove, not one row per WO", async (t) => {
  ...
  t.ok(
    lines[2].startsWith("IVI-3-0001,") && lines[2].includes("HB-NEW"),
    "second row shares the same WO code",
  );
});
...
test("handlers: exportWorkOrdersBulk 404s when none of the ids match", async (t) => {
  ...
  await t.exception(
    () =>
      handlers.exportWorkOrdersBulk(
        ctx,
        { query: { ids: "nope-1,nope-2" } },
        mkRep(),
      ),
  ...
  const ctx = createMockCtxWithOrks(
    ...
    async (_k, method, params) => {
      ...
      return [
        {
          ts: 1,
          changes: { status: { oldValue: "open", newValue: "closed" } },
        },
      ];
    },
  );
